  ...
  private function getRelatedResources($key) {
	...
	public static function getSearchDetails() {
		...
		if ($search['currentStartDate']) {
			...
			if (!$search['currentEndDate']) {
				$searchDisplay[] = _("Subsription valid on or after: ") . $search['currentStartDate'];
			} else {
				$searchDisplay[] = _("Subscription valid between: ") . $search['currentStartDate'] . _(" and ") . $search['currentEndDate'];
			}
		}
	...
	public function searchQuery($whereAdd, $orderBy = '', $limit = '', $count = false) {
		...
		if ($count) {
			...
			$select = "SELECT R.resourceID, R.titleText, AT.shortName acquisitionType, R.createLoginID, CU.firstName, CU.lastName,
							  R.createDate, R.currentStartDate, R.currentEndDate, S.shortName status,
