...
class Resource extends DatabaseObject {
	...
	public function export($whereAdd, $orderBy) {
		...
		$query = "SELECT R.resourceID, R.titleText, AT.shortName acquisitionType, CONCAT_WS(' ', CU.firstName, CU.lastName) createName,
                        ...
                        FORMAT(COALESCE(RPAY.priceTaxExcluded, 0) / 100, " . return_sql_decimals() . " ," . return_sql_locale()  . ") as priceTaxExcluded,
                        FORMAT(COALESCE(RPAY.taxRate, 0) / 100, " . return_sql_decimals() . " ," . return_sql_locale()  . ") as taxRate,
                        FORMAT(COALESCE(RPAY.priceTaxIncluded, 0) / 100, " . return_sql_decimals() . " ," . return_sql_locale()  . ") as priceTaxIncluded,
                        FORMAT(COALESCE(RPAY.paymentAmount, 0) / 100, " . return_sql_decimals() . " ," . return_sql_locale()  . ") as paymentAmount,
